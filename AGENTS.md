...
- **为什么带基线**：2026-09-18 实测当前 HEAD 默认集报 **88 条**（bin 44 个唯一位置 + test
  ...
  **条数以 `tools/check-clippy.mjs` 的 `BASELINE_ALLOW` 为准**（该脚本的运行期消息
  也从数组长度派生，不是硬编码）。逐批进度见 `docs/code-review/后续修复计划_2026-09-17.md`
  的 §7.14 / §7.15——**本文件有意不写条数**，与上一条同理（写死必然漂移）。
...
- **活指针正则必须覆盖「任意 `§N.x`」**，不能只写 `§2.x` / `§x.x`——后者会漏掉 `§6.x`，
  而主方案 §9.4 的手工体检正是因此**漏报**（它当时自称「0 处活指针」）。
- **必须区分「使用」与「提及」**：`§N.x` 落在 `「…」` 内是**引用旧文本**，不计硬失败
  （本仓复核记录大量写「详见 §2.x」来说明那个**已修好**的断指针）；首次跑出 16 处，
...
⚠️ **边界**：本工具只能做「指针存在吗」与「裸占位符」的机械检出；§8.3 第 18 条要求的
