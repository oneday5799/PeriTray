...
- 音频设备右键菜单支持重命名、隐藏和设置快捷键（可为设备录制全局快捷键，一键快速切换默认输出设备；多个设备可共用同一快捷键，按下时按设备列表顺序循环切换）
...
- **快捷键设置**：打开设备信息/音量控制快捷键、音量控制快捷键（提高/降低/静音）、输出设备快捷键集中管理（在音量控制页为设备设置的快捷键，即使设备断开也保留，支持修改、清除、删除；多个设备可共用同一快捷键，按下时按设备列表顺序循环切换）
...
- 输出设备全局快捷键（为音频输出设备录制的快捷键，触发后快速切换默认输出设备；多个设备可共用同一快捷键，按下时按设备列表顺序循环切换）
