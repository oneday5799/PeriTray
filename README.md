...
- **音量控制**：切换默认输出设备、调节音量、静音；静音锁定开关开启后点击图标即永久静音（红色图标），拖到 0 为临时静音（拖动音量条自动恢复）；按应用调节会话音量，滑块支持滚轮微调与实时数值 tooltip；设备右键菜单支持重命名、隐藏与录制全局快捷键
...
- **设置页**：涵盖通用（开机自启、默认打开页面、硬件加速、日志、关机音量、更新检测）、快捷键、设备信息（过滤/去重/分组）、音量控制等设置，支持深色主题
