...
## 七、最终评估（2026-09-08 终验）

> 逐项核实源码确认跳过原因仍然成立，无遗漏需补做项。

| # | 说明 | 跳过原因 | 终验结论 |
|---|------|---------|---------|
| **#20** | `hidden_groups.includes` 假设数组 | 后端 `Vec<String>` + serde 保证恒序列化为 JSON 数组 | ✅ 跳过有效——无实际 bug |
| **#22** | 蓝牙电量 SetupDi 批次化 | 1-3 台 BT × ~100 系统设备，每 5 分钟一次 | ✅ 跳过有效——O(N×M) 中 N 极小，总开销 trivial |
| **#24** | `resolve_toast_icon` 每轮写盘 | 仅在 low_battery_notify 启用且阈值非空时触发，每 10s 写一次嵌入式 PNG | ⚠️ 跳过有效——写盘开销小，但可改为首次 toast 时按需写入（锦上添花，非必要） |
| **#26** | `fs::metadata` 每轮轮询 | 单次 `GetFileAttributesExW` 调用，未变化时 early return | ✅ 跳过有效——微秒级系统调用 |
| **#32** | `set_shutdown_volumes` 按设备名匹配 | 仅在 `WM_ENDSESSION` 时调用一次 | ✅ 跳过有效——关机时单次执行，无性能 concern |
| **#34** | 托盘短任务 spawn 线程 | 有意设计避免阻塞事件循环，代码注释明确说明 | ✅ 跳过有效——架构决策，非缺陷 |
| **#35** | 锁风格混用 | `lock_unpoisoned` / `.lock().ok()` / `.unwrap()` 三种均能正确处理中毒 | ✅ 跳过有效——纯风格一致性，不影响功能 |
| **#37** | WinHTTP RAII guard | 每次会话仅 1-2 次更新检查，手动清理路径完整 | ✅ 跳过有效——低频操作，手动 CloseHandle 无遗漏 |
| **#38** | app_icon 负缓存 + 盘符映射 | LRU 缓存（256 容量）已生效，按需解析非热路径 | ✅ 跳过有效——缓存已覆盖 |
| **#39** | classify 品牌名冗余 | `match_usb_keyword` 与 `wireless_24g::drivers` 服务于不同分类路径 | ✅ 跳过有效——非冗余，各有用途 |
| **#42** | 双 toast 统一 | `toast.rs`（无回调/自动隐藏）与 `build_toast`（有点击回调）API 需求不同，不冲突 | ✅ 跳过有效——各司其职，无实际冲突 |

### 结论

**全部 11 项跳过原因经终验确认仍然成立，无需补做。** 仅 #24 存在边际优化空间（图标按需写入），但收益极低，不影响功能正确性和用户体验。

优化方案实施完毕。feat/optimization-batch1 分支共 8 批 37 项落地，覆盖全部真实缺陷（批一 11/11）、音频性能（批二 3/3）、前端稳定性（批三 11/12）、查询效率（批四 5/8）和清理优化（批五 7/14）。
